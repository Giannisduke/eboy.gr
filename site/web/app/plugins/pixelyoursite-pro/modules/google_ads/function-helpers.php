...
function getWooFullItemId( $item_id ) {
    ...
    if(isWPMLActive() && PixelYourSite\GATags()->getOption( 'woo_wpml_unified_id' )) {
        $wpml_product_id = apply_filters('wpml_original_element_id', NULL, $item_id);
        if ($wpml_product_id) {
            $item_id = $wpml_product_id;
        }
    }
...
function getConversionIDs($eventKey) {
    ...
    foreach ($tag_ids as $key => $tag_id) {
        if (!empty($labels) && isset($labels[$tag_id])) {
            $conversion_ids[] = $tag_id . '/' . $labels[$tag_id];
        }
        elseif ($type_conversion_label != 'conversion'){
