	...
	public function update( $args = null ) {
		...
		if ( !empty( $args[ 'triggers' ] ) ) {
			foreach ( $args[ 'triggers' ] as $data_trigger ) {
				...
				if ( $trigger_type === 'post_type' || $trigger_type === 'home_page' ) {
					$saving_trigger = true;
				}
				...
				// url filters
				if ( in_array( $trigger_type, array(
						'url_click',
						'css_click',
						'css_mouseover',
						'scroll_pos'
					) ) && isset( $data_trigger[ 'url_filter_triggers' ] ) && is_array( $data_trigger[ 'url_filter_triggers' ] ) ) {
					$url_filters = array();

					foreach ( $data_trigger[ 'url_filter_triggers' ] as $url_filter_trigger ) {
						if ( !empty( $url_filter_trigger[ 'value' ] ) ) {
							$url_filters[] = array(
								'rule'  => null,
								'value' => sanitize_text_field( $url_filter_trigger[ 'value' ] ),
							);
						}
					}

					$trigger->updateParam( 'url_filters', $url_filters );
				}
