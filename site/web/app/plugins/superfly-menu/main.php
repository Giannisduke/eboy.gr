<?php
/*
Plugin Name: Superfly Menu
Plugin URI: http://superfly.looks-awesome.com
Description: Off-canvas hamburger menu for WordPress
Version: 5.0.24
Author: Looks Awesome
Author URI: ht  tp://looks-awesome.com
License: Commercial License
Text Domain: superfly-menu
Domain Path: /lang
*/

global $sfm_options;
global $sfm_admin_page;
global $sfm_menus_page;

$sfm_plugin = array(
    'name' => 'superfly',
    'label' => 'Superfly',
    'prefix' => 'sf',
    'demo_site' => 'http://superfly.looks-awesome.com',
    'version' => '5.0.24',
    'require_php' => '5.3.0',
    'require_ext' => array('zip'),
    'namespace' => 'Superfly',
    'base' => '',
    'base_url' => '',
    'mode' => ''
);

require __DIR__ . '/includes/Activator.php';

$sfm_activator = new LA_Supefly_App_Activator( $sfm_plugin );
add_action('admin_init', array('LA_Supefly_App_Activator', 'check'));
if (!LA_Supefly_App_Activator::compatible()) {
    return;
}

register_activation_hook(__FILE__, 'LA_Supefly_App_Activator::activate');

if (!defined('SFM_VERSION_KEY')) {
    define('SFM_VERSION_KEY', 'SF_version');
}

if (!defined('SFM_VERSION_NUM')) {
    define('SFM_VERSION_NUM', '5.0.24');
}

if (!defined('SFM_ITEM_META')) {
    define('SFM_ITEM_META', '_sf_settings');
}

require __DIR__ . '/includes/vendor/looks_awesome/envato_api/Purchase.php';

add_option(SFM_VERSION_KEY, SFM_VERSION_NUM);

load_plugin_textdomain('superfly-menu', false, basename(dirname(__FILE__)) . '/lang');

include_once(dirname(__FILE__) . '/settings.php');
include_once(dirname(__FILE__) . '/customizer.php');

if (!class_exists('LA_GoogleFonts')) {
    include_once(dirname(__FILE__) . '/includes/vendor/looks_awesome/google_fonts/GoogleFonts.php');
}


$fonts = trailingslashit(plugin_dir_path(__FILE__) . 'fonts');
if ( !class_exists('LA_IconManager') ) {

    $sf_is_ajax = defined( 'DOING_AJAX' ) && DOING_AJAX && ( strpos( $_REQUEST['action'], 'laim_') !== false || strpos( $_REQUEST['action'], 'sf_preview') !== false) ;
    if ( !is_admin() || (strpos( $_SERVER['REQUEST_URI'],'page=superfly-menu-options') !== false ) || (strpos($_SERVER['REQUEST_URI'],'wp-admin/customize.php') !== false || strpos($_SERVER['REQUEST_URI'],'nav-menus.php') !== false || $sf_is_ajax)) {
        include_once(dirname(__FILE__) . '/includes/vendor/looks_awesome/icon_manager/IconManager.php');
        $sf_la_icon_manager = LA_IconManager::getInstance($fonts);
        register_activation_hook(__FILE__, array($sf_la_icon_manager, 'addDefaultFonts'));
        register_deactivation_hook(__FILE__, 'LA_IconManager::deleteOption');
    }
}

$env = array(
    'mode' => 'dev',
    'google_api_key' => '',
);
if (file_exists(plugin_dir_path(__FILE__) . 'env.json')) {
    $env = json_decode(file_get_contents(plugin_dir_path(__FILE__) . 'env.json'), true);
    if (!defined('SF_MODE')) {
        $mode = $env['mode'] ? $env['mode'] : 'dev';
        define('SF_MODE', $mode);
    }
    if (!defined('SF_GOOGLE_API_KEY') && $env['mode'] == 'dev') {
        $key = $env['google_api_key'] ? $env['google_api_key'] : '';
        define('SF_GOOGLE_API_KEY', $key);
    }
}

add_action('wp_enqueue_scripts', 'sfm_scripts');

add_action('admin_menu', 'sf_menu');

function sf_menu()
{
    global $sfm_admin_page;
    $sfm_admin_page = add_menu_page(
        'Superfly Menu Options',
        'Superfly Menu',
        'manage_options',
        'superfly-menu-options',
        'sfm_page',
        'data:image/svg+xml;base64,',
        22
    );
    // toplevel_page_superfly-menu-options
}

add_action('admin_print_styles-nav-menus.php', 'sfm_menus_admin');

/**
 * Appearance/Menus page in the WP Admin
 */
function sfm_menus_admin()
{
    if (SF_MODE === 'dev') {
        wp_enqueue_script(
            'sf-menus-admin-js',
            plugins_url('/js/menu-admin.js', __FILE__),
            array('jquery', 'wp-color-picker'),
            SFM_VERSION_NUM,
            true
        );
        wp_enqueue_script('awesome-ajax', plugins_url('/js/vendor/looks_awesome/common/ajax.js', __FILE__));
        wp_enqueue_script('awesome-util', plugins_url('/js/vendor/looks_awesome/common/util.js', __FILE__));
        wp_enqueue_style('sf-admin-font-awesome', plugins_url('/css/fa.min.css', __FILE__));
        wp_enqueue_style('sf-menus-admin-css', plugins_url('/css/menu-admin.css', __FILE__));
        $handle = 'awesome-ajax';
    } else {
        wp_enqueue_script(
            'sf-menus-admin-js',
            plugins_url('/js/admin_menu.min.js', __FILE__),
            array('jquery', 'wp-color-picker'),
            SFM_VERSION_NUM
        );
        wp_enqueue_style(
            'sf-menus-admin-css',
            plugins_url('/css/admin_menu.min.css', __FILE__),
            array(),
            SFM_VERSION_NUM,
            'all'
        );
        $handle = 'sf-menus-admin-js';
    }

    $fonts = trailingslashit(plugin_dir_path(__FILE__) . 'fonts');

    if ( class_exists('LA_IconManager') ) {
        $sf_la_icon_manager = LA_IconManager::getInstance($fonts);
        $sf_la_icon_manager->enqueueAdminScripts();
    }

    wp_enqueue_media();
    wp_enqueue_style('lato-font', '//fonts.googleapis.com/css?family=Lato:300,400,600');
    wp_enqueue_style('wp-color-picker');

    $sf_menu_data = sfm_get_menus_data();

    wp_localize_script('sf-menus-admin-js', 'sf_menus_data', $sf_menu_data);
    wp_localize_script(
        $handle,
        'sf_menus_meta',
        array(
            'ajax_url' => admin_url('admin-ajax.php'),
        )
    );

    // Icon Manager
    wp_localize_script(
        $handle,
        'laim_localize',
        array(
            'ajax_nonce' => wp_create_nonce('sf'),
            'ajaxurl' => admin_url('admin-ajax.php'),
        )
    );
}


add_action('wp_ajax_sf_save_item', 'sfm_save_item_fn');
add_action('wp_ajax_sf_get_custom_content', 'sfm_get_custom_content_fn');
add_action('wp_ajax_sf_validate_envato_license', 'sfm_validate_envato_license_fn');

function sfm_validate_envato_license_fn() {
    if (isset($_POST['code'])) {
        $json = EnvatoApi\Purchase::verify($_POST['code']);

        if (is_object($json)) {
            wp_send_json($json);
        }
    }
}

function sfm_get_custom_content_fn() {
    if ( isset ( $_POST['settings'] ) ) {
        // can contain HTML tags

        $serialized_settings = $_POST[ 'settings' ];
        $settings = array();
        parse_str( $serialized_settings, $settings );

        $response = array();
        $response[ 'content' ] = do_shortcode( $settings['content'] );

        wp_send_json( $response );
    }
}

function sfm_save_item_fn() {

    if ( isset ( $_POST['id'] ) && isset ( $_POST['settings'] )) { // deprecated

        $menu_item_id = $_POST['id'];
        $serialized_settings = $_POST['settings'];
        $result = update_post_meta($menu_item_id, SFM_ITEM_META, $serialized_settings);
        $response = array();
        $response['settings'] = $serialized_settings;
        $response['menu_item_id'] = $menu_item_id;
        $response['meta'] = $result;

        wp_send_json( $response );

    } else if ( isset( $_POST['changeset'] ) ) { // new API for customizer

        $menu_items = $_POST['changeset'];
        $response = array();

        if ( is_array( $menu_items ) ) {
            foreach ( $menu_items as $id => $settings ) {
                $result = update_post_meta( $id, SFM_ITEM_META, $settings );
                $response[ $id ] = $result;
            }
        }

        wp_send_json( $response );

    } else {
        echo -1;
        die();
    }

}

function sfm_register_widget_area()
{

    if (function_exists('register_sidebar')) {
        register_sidebar(
            array(
                'name' => 'Superfly Widget Area',
                'id' => 'sf_sidebar_widget_area',
                'description' => __(
                    'Widgets in this area will be shown in Superfly sidebar under the menu',
                    'superfly-menu'
                ),
                'before_widget' => '<div class="widget">',
                'after_widget' => '</div>',
                'before_title' => '<h1 class="title">',
                'after_title' => '</h1>',
            )
        );
    }

}

add_action('wp_loaded', 'sfm_register_widget_area');

function sfm_get_menus_data($menu_id = -1)
{

    if ($menu_id == -1) {
        global $nav_menu_selected_id;
        $menu_id = $nav_menu_selected_id;
    }

    if ($menu_id == 0) {
        return array();
    }

    $sf_menus_data = array();
    $menu_items = wp_get_nav_menu_items($menu_id, array('post_status' => 'any'));

    if (is_array($menu_items)) {
        foreach ($menu_items as $item) {
            $_item_settings = sfm_get_menu_item_data($item->ID);
            if ($_item_settings != '') {
                $sf_menus_data[$item->ID] = $_item_settings;
            }
        }
    }

    //shiftp( $sf_menus_data );
    return $sf_menus_data;
}

function sfm_get_menu_item_data($item_id)
{
    $meta = get_post_meta($item_id, SFM_ITEM_META, true);

    //Add URL for image
    if (!empty($meta['item_image'])) {
        $src = wp_get_attachment_image_src($meta['item_image']);
        if ($src) {
            $meta['item_image_url'] = $src[0];
            $meta['item_image_edit'] = get_edit_post_link($meta['item_image'], 'raw');
        }
    }

    return $meta;
}

/**
 * Settings page in the WP Admin
 */
function sfm_page()
{

    if ( !current_user_can('manage_options') ) {
        wp_die( __('You do not have sufficient permissions to access this page.', 'superfly-menu') );
    }

    if ( SF_MODE === 'dev') {
        // load fresh Google Fonts list
//        add_filter('styles_google_font_api', create_function('', "return '" . SF_GOOGLE_API_KEY . "';"));
        add_filter('styles_google_font_api', function() { return SF_GOOGLE_API_KEY; } );
        $google_fonts = new LA_GoogleFonts();

        wp_enqueue_script(
            'sf_admin',
            plugins_url('/js/admin.js', __FILE__),
            array('jquery', 'underscore', 'backbone', 'fontselect'),
            SFM_VERSION_NUM
        );

        wp_enqueue_script(
            'backbone-stickit',
            plugins_url('/js/vendor/backbone/backbone.stickit.js', __FILE__),
            array(),
            '0.9.2'
        );

        wp_enqueue_script('sf_admin_bindings', plugins_url('/js/admin-bindings.js', __FILE__), array('backbone-stickit'), SFM_VERSION_NUM);
        wp_enqueue_script('tinycolor', plugins_url('/js/tinycolor.js', __FILE__), array(), SFM_VERSION_NUM);
        wp_enqueue_script(
            'sf_colorpickersliders',
            plugins_url('/js/jquery.colorpickersliders.js', __FILE__),
            array(),
            SFM_VERSION_NUM
        );
        wp_enqueue_script('awesome-ajax', plugins_url('/js/vendor/looks_awesome/common/ajax.js', __FILE__));
        wp_enqueue_script('awesome-util', plugins_url('/js/vendor/looks_awesome/common/util.js', __FILE__));
        wp_enqueue_script(
            'fontselect',
            plugins_url('/js/vendor/tommoor/fontselect-jquery-plugin/jquery.fontselect.js', __FILE__)
        );
        $handle = 'awesome-ajax';

        wp_enqueue_style('colorpickersliders-ui-css', plugins_url('/css/jquery.colorpickersliders.css', __FILE__));
        wp_enqueue_style(
            'fontselect-css',
            plugins_url('/js/vendor/tommoor/fontselect-jquery-plugin/fontselect.css', __FILE__)
        );
        wp_enqueue_style('sf-admin-css', plugins_url('/css/admin.css', __FILE__));
    } else {
        wp_enqueue_script('sf-admin-js', plugins_url('/js/admin.min.js', __FILE__), array(), SFM_VERSION_NUM);
        wp_enqueue_style('sf-admin-css', plugins_url('/css/admin.min.css', __FILE__), array(), SFM_VERSION_NUM, 'all');
        $handle = 'sf-admin-js';
    }

    wp_enqueue_media();
    wp_enqueue_script('wp-ajax-response');
    wp_enqueue_style('lato-font', '//fonts.googleapis.com/css?family=Lato:300,400');

    $fonts = trailingslashit(plugin_dir_path(__FILE__) . 'fonts');

    if ( class_exists('LA_IconManager') ) {
        $sf_la_icon_manager = LA_IconManager::getInstance($fonts);
        $sf_la_icon_manager->enqueueAdminScripts();
    }

    wp_localize_script(
        $handle,
        'laim_localize',
        array(
            'ajax_nonce' => wp_create_nonce('sf'),
            'ajaxurl' => admin_url('admin-ajax.php'),
        )
    );

    include_once(dirname(__FILE__) . '/options-page.php');
}


add_filter('plugin_action_links_superfly-menu/main.php', 'sfm_plugin_action_links', 10, 1);

function sfm_plugin_action_links($links)
{
    $settings_page = add_query_arg(array('page' => 'superfly-menu-options'), admin_url('options-general.php'));
    $settings_link = '<a href="' . esc_url($settings_page) . '">' . __('Settings', 'sf') . '</a>';
    array_unshift($links, $settings_link);

    return $links;
}

add_action( 'wp_head', 'sfm_add_dynamic_content', 10 );
add_action( 'wp_head', 'sfm_main_html_template', 10 );

add_action( 'admin_post_sfm_export_settings', 'sfm_export_settings' );
add_action( 'admin_post_sfm_import_settings', 'sfm_import_settings' );

function sfm_export_settings () {
    // Handle request then generate response using echo or leaving PHP and using HTML
    status_header(200);

    $options = sf_get_options();
    error_log( gettype($options) );
    $json_body = $options; // arrays are clones by copy

    unset( $json_body[ 'sf_active_menu' ] );
    unset( $json_body[ 'sf_display' ] );
    unset( $json_body[ 'sf_alternative_menu' ] );
    unset( $json_body[ 'sf_tab_logo' ] );
    unset( $json_body[ 'locations' ] );

    header('Content-Type: application/json');
    header('Content-Disposition: attachment; filename=superfly_settings_' . str_replace('https://', '', str_replace('http://', '', get_bloginfo( 'url' ) ) )   . '.json');
    header('Pragma: no-cache');

    echo json_encode( $json_body );
}

function sfm_import_settings () {
    $import = $_POST['data'];

    // sanitize/validate all values

    $sfm_options = get_option('sf_options');

    foreach( $import as $setting => &$value ) {

        if ( $setting === 'sf_copy' || $setting === 'sf_above_logo' || $setting === 'sf_under_logo' || !is_string( $value ) ) { // these can contain HTML
            continue;
        } else {
            $value = filter_var( trim( $value ), FILTER_SANITIZE_STRING );
            if ( isset( $sfm_options[ $setting ] ) ) $sfm_options[ $setting ] = $value;
        }
    }

    update_option('sf_options', $sfm_options);

    header('Content-Type: application/json');
    echo json_encode( $import );
}

function sfm_main_html_template()
{
    global $sf_show;
    global $current_page_sf_menu;

    $options = sf_get_options();
    if ( $options['sf_test_mode'] === 'yes' && !current_user_can( 'manage_options' ) ) {
        return;
    }
    if ( isset( $sf_show ) && $sf_show && isset( $current_page_sf_menu ) && sfm_exclude_builders() ) {
        ob_start();
        include_once(dirname(__FILE__) . '/superfly-menu.php');
        $superflyHTML = ob_get_contents();
        ob_end_clean();

        echo "<script type='text/javascript'>var SFM_template =" . json_encode($superflyHTML) . "</script>";
    }
}

function sfm_exclude_builders ( ) {
    $oxygen_builder = isset( $_GET["ct_builder"] ) && empty( $_GET["oxygen_iframe"] );
    return empty($_GET["cornerstone"]) && !$oxygen_builder;
}

function sfm_add_dynamic_content()
{
    global $sf_show;
    global $current_page_sf_menu;

    $options = sf_get_options();
    if ( $options['sf_test_mode'] === 'yes' && !current_user_can('manage_options') ) {
        return;
    }

    if ( isset( $sf_show ) && $sf_show && isset( $current_page_sf_menu ) && sfm_exclude_builders() ) {
        include_once(dirname(__FILE__) . '/superfly-dynamic.php');
    }
}

function sfm_scripts()
{
    global $sf_show;
    global $sf_menu_data;
    global $current_page_sf_menu;

    $options = sf_get_options();
    if ($options['sf_test_mode'] === 'yes' && !current_user_can('manage_options')) {
        return;
    }

    $post_id = get_queried_object_id();
    $ismobile = wp_is_mobile();
    $sf_show = sfm_check_display_rule(json_decode($options['sf_display']), $ismobile, $post_id);

    //sfm_debug_to_console($sf_show);

    if ( $sf_show && isset( $current_page_sf_menu ) && sfm_exclude_builders()  ) {
        if ( SF_MODE === 'dev' || $options['sf_dev'] == 'yes' ) {
            wp_enqueue_script(
                'sf_main',
                plugins_url('/js/superfly-menu.js', __FILE__),
                array('jquery'),
                SFM_VERSION_NUM,
                false
            );
            wp_enqueue_style('sf_styles', plugins_url('/css/superfly-menu.css', __FILE__));
        } else {
            wp_enqueue_script(
                'sf_main',
                plugins_url('/js/public.min.js', __FILE__),
                array('jquery'),
                SFM_VERSION_NUM,
                false
            );
            wp_enqueue_style('sf_styles', plugins_url('/css/public.min.css', __FILE__));
        }

        $social = array();
        $networks = array(
            'facebook',
            'twitter',
            'linkedin',
            'gplus',
            'instagram',
            'pinterest',
            'flickr',
            'dribbble',
            'youtube',
            'vimeo',
            'soundcloud',
            'email',
            'skype',
            'rss',
        );

        foreach ($networks as $network) {
            if (!empty($options['sf_' . $network])) {
                $social[$network] = $options['sf_' . $network];
            }
        }

        $sf_menu_data = sfm_get_menus_data( $current_page_sf_menu );

        $fonts = trailingslashit(plugin_dir_path(__FILE__) . 'fonts');

        if ( class_exists('LA_IconManager') ) {
            $sf_la_icon_manager = LA_IconManager::getInstance($fonts);
            $sf_la_icon_manager->enqueuePublicScripts();
        }

        wp_localize_script(
            'sf_main',
            'SF_Opts',
            array(
                'wp_menu_id' => $current_page_sf_menu,
                'social' => $social,
                'search' => $options['sf_search'],
                'blur' => $options['sf_blur_content'],
                'fade' => $options['sf_transition'],
                'test_mode' => $options['sf_test_mode'],
                'hide_def' => $options['sf_hide_def'],
                'mob_nav' => $options['sf_mob_nav'],
                'dynamic' => $options['sf_fs_dynamic'],
                'parent_ignore' => $options['sf_parent_ignore'],
                'sidebar_style' => $options['sf_sidebar_style'],
                'sidebar_behaviour' => $options['sf_sidebar_behaviour'],
                'alt_menu' => $options['sf_alternative_menu'],
                'sidebar_pos' => $options['sf_sidebar_pos'],
                'width_panel_1' => ($options['sf_sidebar_style'] == 'toolbar' ? 100 : $options['sf_width_panel_1']),
                'width_panel_2' => $options['sf_width_panel_2'],
                'width_panel_3' => $options['sf_width_panel_3'],
                'width_panel_4' => $options['sf_width_panel_4'],
                'base_color' => $options['sf_bg_color_panel_1'],
                'opening_type' => $options['sf_opening_type'],
                'sub_type' => $options['sf_sub_type'],
                'video_bg' => $options['sf_video_bg'],
                'video_mob' => $options['sf_video_mob'],
                'video_preload' => $options['sf_video_preload'],
                'sub_mob_type' => $options['sf_sub_mob_type'],
                'sub_opening_type' => ( $options['sf_sub_type'] != 'flyout' ? 'click' : $options['sf_sub_opening_type'] ),
                'label' => $options['sf_label_style'],
                'label_top' => $options['sf_label_top'],
                'label_size' => $options['sf_label_size'],
                'label_vis' => $options['sf_label_vis'],
                'item_padding' => $options['sf_padding'],
                'bg' => $options['sf_image_bg'],
                'path' => plugins_url('/img/', __FILE__),
                'menu' => $options['sf_active_menu'],
                'togglers' => $options['sf_togglers'],
                'subMenuSupport' => (($ismobile && !$options['sf_submenu_mob'] || $options['sf_sidebar_style'] === 'skew') ? false : $options['sf_submenu_support']),
                'subMenuSelector' => $options['sf_submenu_classes'],
                'eventsInterval' => $options['sf_interval'],
                'activeClassSelector' => 'current-menu-item',
                'allowedTags' => 'DIV, NAV, UL, OL, LI, A, P, H1, H2, H3, H4, SPAN',
                'menuData' => $sf_menu_data,
                'siteBase' => site_url(),
                'plugin_ver' => SFM_VERSION_NUM
            )
        );

        if ($options['sf_font'] !== 'inherit') {
            wp_register_style('sf-google-font', '//fonts.googleapis.com/css?family=' . $options['sf_font']);
            wp_enqueue_style('sf-google-font');
        }

        if ($options['sf_c_font'] !== 'inherit') {
            wp_register_style('sf-google-font-subheader', '//fonts.googleapis.com/css?family=' . $options['sf_c_font']);
            wp_enqueue_style('sf-google-font-subheader');
        }
    }
}

function sfm_is_mobile()
{
    return preg_match(
        "/(android|avantgo|blackberry|bolt|boost|cricket|docomo|fone|hiptop|mini|mobi|palm|phone|pie|tablet|up\.browser|up\.link|webos|wos)/i",
        $_SERVER["HTTP_USER_AGENT"]
    );
}

function sfm_get_lang_id($id, $type = 'page')
{
    if (function_exists('icl_object_id')) {
        $id = icl_object_id($id, $type, true);
    }

    return $id;
}

function sfm_check_location($opt, $post_id)
{

    global $current_page_sf_menu;

    if (is_home()) {

        $show = isset($opt->location->wp_pages->home);
        $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'home', $post_id);
        if (!$show && $post_id) {
            $show = isset($opt->location->pages->$post_id);
            $current_page_sf_menu = sfm_check_against_rule('pages', $post_id, $post_id);
        }

        // check if blog page is front page too
        if (!$show && is_front_page() /*&& isset($opt['page-front'])*/) {
            $show = isset($opt->location->wp_pages->front);
            $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'front', $post_id);
        }

    } else {
        if (is_front_page()) {

            $show = isset($opt->location->wp_pages->front);
            $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'front', $post_id);

            if (!$show && $post_id) {
                $show = isset($opt->location->pages->$post_id);
                $current_page_sf_menu = sfm_check_against_rule('pages', $post_id, $post_id);
            }
        } else {
            if (is_category()) {
                //browser()->log  ( 'cat' );
                //browser()->log  ( get_query_var('cat') );
                $catid = get_query_var('cat');
                $show = isset($opt->location->cats->$catid);
                $current_page_sf_menu = sfm_check_against_rule('cats', $catid, $post_id);

            } /*else if ( is_tax() ) {
				$term = get_queried_object();
				$tax = $term->taxonomy;
				$show = isset($opt->location->cats->$tax);
				unset($term);
				unset($tax);
			} else if ( is_post_type_archive() ) {
				$type = get_post_type();
				$show = isset($opt['type-'. $type .'-archive']) ? $opt['type-'. $type .'-archive'] : false;
			}*/ else {
                if (is_archive()) {
                    //browser()->log  ( 'archive' );

                    $show = isset($opt->location->wp_pages->archive);
                    $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'archive', $post_id);

                } else {
                    if (is_single()) {
                        //browser()->log  ( 'single' );

                        $type = get_post_type();
                        $show = isset($opt->location->wp_pages->single);
                        $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'single', $post_id);
                        if (!$show && $type != 'page' && $type != 'post') {
                            $show = isset($opt->location->cposts->$type);
                            $current_page_sf_menu = sfm_check_against_rule('cposts', $type, $post_id);
                        }

                        if (!$show) {
                            $cats = get_the_category();
                            foreach ($cats as $cat) {
                                if ($show) {
                                    break;
                                }
                                $c_id = sfm_get_lang_id($cat->cat_ID, 'category');
                                $show = isset($opt->location->cats->$c_id);
                                $current_page_sf_menu = sfm_check_against_rule('cats', $c_id, $post_id);
                                unset($c_id);
                                unset($cat);
                            }
                        }

                    } else {
                        if (is_404()) {
                            $show = isset($opt->location->wp_pages->forbidden);
                            $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'forbidden', $post_id);

                        } else {
                            if (is_search()) {
                                $show = isset($opt->location->wp_pages->search);
                                $current_page_sf_menu = sfm_check_against_rule('wp_pages', 'search', $post_id);

                            } else {
                                if ($post_id) {
                                    $show = isset($opt->location->pages->$post_id);
                                    $current_page_sf_menu = sfm_check_against_rule('pages', $post_id, $post_id);

                                } else {
                                    $show = false;
                                    $current_page_sf_menu = sfm_check_against_rule('pages', '', $post_id);
                                }
                            }
                        }
                    }
                }
            }
        }
    }

    if ($post_id && !$show && isset($opt->location->ids) && !empty($opt->location->ids)) {

        $other_ids = $opt->location->ids;

        foreach ($other_ids as $other_id) {
            if ($post_id == (int)$other_id) {
                $show = true;
            }
        }
    }

    if (!$show && defined('ICL_LANGUAGE_CODE')) {
        // check for WPML widgets
        $lang = ICL_LANGUAGE_CODE;
        $show = isset($opt->location->langs->$lang);
//        $current_page_sf_menu = sfm_check_against_rule('langs', $lang);
    }


    if (!isset($show)) {
        //browser()->log  ( '!isset($show)' );
        $show = false;
    }


    return $show;
}

function sfm_check_against_rule($first_level, $sec_level, $post_id = false)
{
    $options = sf_get_options();
    $locations = json_decode($options['sf_active_menu']);
    global $current_page_sf_menu;

    $isDef = false;
    if (gettype($locations) == 'object') { /* migration */
        foreach ($locations as $menu_object) {

            $current_loc = $menu_object->loc;

            if (isset($current_loc->pages) && !count( is_object( $current_loc->pages ) ? get_object_vars( $current_loc->pages ) : $current_loc->pages ) &&
                isset($current_loc->cposts) && !count( is_object( $current_loc->cposts ) ? get_object_vars( $current_loc->cposts ) : $current_loc->cposts ) &&
                isset($current_loc->cats) && !count( is_object( $current_loc->cats ) ? get_object_vars( $current_loc->cats ) : $current_loc->cats ) &&
                isset($current_loc->taxes) && !count( is_object( $current_loc->taxes ) ? get_object_vars( $current_loc->taxes ) : $current_loc->taxes ) &&
                isset($current_loc->wp_pages) && !count( is_object( $current_loc->wp_pages ) ? get_object_vars( $current_loc->wp_pages ) : $current_loc->wp_pages ) &&
                empty($current_loc->ids[0])
            ) {
                $onlyLang = true;
            }

            if (defined('ICL_LANGUAGE_CODE')) {
                // check for WPML widgets
                $pageLang = ICL_LANGUAGE_CODE;
                if (isset($onlyLang) && $current_loc->langs->$pageLang) {
                    return $menu_object->term_id;
                }
            }

            if (isset($current_loc->$first_level->$sec_level)) {

                if (isset($pageLang)) {
                    if ($current_loc->langs->$pageLang) return $menu_object->term_id;
                } else {
                    return $menu_object->term_id;
                }

            }
            if (isset($menu_object->isDef) && $menu_object->isDef) {
                $isDef = $menu_object->term_id;
            }
            if ($post_id !== false && isset($current_loc->ids) && !empty($current_loc->ids)) {
                $other_ids = $current_loc->ids;

                foreach ($other_ids as $other_id) {
                    if ($other_id !== '' && $post_id == (int)$other_id) {
                        if (isset($pageLang)) {
                            if ($current_loc->langs->$pageLang) return $menu_object->term_id;
                        } else {
                            return $menu_object->term_id;
                        }
                    }
                }
            }
        }
    }

    if ($isDef) {
        return $isDef;
    }
}

function sfm_check_display_rule($opt, $isMobile, $post_id)
{
    $show = sfm_check_location($opt, $post_id);

    if ($show && $opt->rule->exclude || !$show && $opt->rule->include) {
        $show = false;
    } else {
        $show = true;
    }

    $user_ID = is_user_logged_in();

    if (($opt->user->loggedout && $user_ID) || ($opt->user->loggedin && !$user_ID)) {
        $show = false;
    }

    if ($opt->mobile->no && $isMobile) {
        $show = false;
    }

    if ($opt->desktop->no && !$isMobile) {
        $show = false;
    }

    return $show;
}

function sfm_debug_to_console($data)
{
    if (is_array($data) || is_object($data)) {
        echo("<script>console.log('PHP: " . json_encode($data) . "');</script>");
    } else {
        echo("<script>console.log('PHP: " . $data . "');</script>");
    }
}

function sfm_deparam($query)
{
    $map = array();

    if (!empty($query)) {
        $pairs = explode('&', $query);
        $len = sizeof($pairs);
        for ($i = 0; $i < $len; $i += 1) {
            $keyValuePair = explode('=', $pairs[$i]);
            $key = $keyValuePair[0];
            $value = (sizeof($keyValuePair) > 1) ? $keyValuePair[1] : null;
            $map[$key] = $value;
        }
    }

    return $map;
}

function sfm_convert_symbols_custom($text)
{
    $current_user = wp_get_current_user();

    // Logic {{username}}
    if (strpos($text, '{{username}}') !== false) {
        $username = $current_user->display_name;
        $text = str_replace('{{username}}', $username, $text);
    }

    return $text;
}

add_filter('sf_convert_symbols', 'sfm_convert_symbols_custom', 10, 1);